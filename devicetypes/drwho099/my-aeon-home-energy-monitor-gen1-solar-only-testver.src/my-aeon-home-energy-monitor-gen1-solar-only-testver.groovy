 ...
 */
metadata {
	definition (name: "My Aeon Home Energy Monitor Gen1 Solar-only TESTVER", namespace: "Drwho099", author: "jscgs350+")
	{
...
def parse(String description) {
	...
	def statusTextmsg = "Min: ${device.currentState('minWATTS')?.value}\nMax: ${device.currentState('maxWATTS')?.value}"
...
def poll() {
	log.debug "poll()"
	// refresh()              GRH -  skip refresh of device from poll, as they interfered with interval reporting consistency.
}
...
//	zwave.configurationV1.configurationSet(parameterNumber: 255, size: 4, scaledConfigurationValue: 1).format()
	...
	// If parameter 3 is 0, report every XX seconds (for battery) for Reporting Group 3 for the whole device.
